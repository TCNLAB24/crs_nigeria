...
DYNAMIC_SIMS_FOLDER <- file.path(ALL_OUTPUT_FOLDER, "dynamic_risk_simulations", "raw_simulations"); if(!file.exists(DYNAMIC_SIMS_FOLDER)) dir.create(DYNAMIC_SIMS_FOLDER)
...
FIGURES_FOLDER <- file.path("figures", "dynamic_analysis"); if(!file.exists(FIGURES_FOLDER)) dir.create(FIGURES_FOLDER)
```
...
ng1_pop <- ng2_pop %>% 
  ...
  mutate(n=m+f) %>% 
  ungroup()
ng1_zone_mapping <- read.csv(ng1_zone_mapping_path)
